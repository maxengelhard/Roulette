...
const LargeTable = (props) => {
    ...
    const table = tableArr.map((num,key) =>  {
        return (
           <tr key={key}><td><button className={num[0]} onClick={() => props.onClick(props.bet,num[0])}>{label(num[0]) ? label(num[0]) : num[0]}</button></td><td><button className={num[1]} onClick={() => props.onClick(props.bet,num[1])}>{label(num[1]) ? label(num[1]) : num[1]}</button></td><td><button className={num[2]} onClick={() => props.onClick(props.bet,num[2])}>{label(num[2]) ? label(num[2]) : num[2]}</button></td></tr> 
        )
    })
    return (
            <table> 
                <tbody>
    <tr><td colSpan='3'><button className='0' onClick={() => props.onClick(props.bet,0)}>{label(0) ? label(0) : 0}</button></td></tr>
                     {table}
                </tbody>
            </table>
    )
